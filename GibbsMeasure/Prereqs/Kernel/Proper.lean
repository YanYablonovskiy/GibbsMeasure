    ...
    (hf : Integrable[𝓧] f (π x₀)) (hB : MeasurableSet[𝓑] B):
      ∫ x, (B.indicator 1 x) * (f x) ∂(π x₀) = B.indicator 1 x₀ * ∫ x, f x ∂(π x₀) := by
        ...
        · intro c S hmS bpS _
          ...
          rw [integral_indicator_mul_indicator hπ h𝓑𝓧 hmS hB,←mul_assoc]
        ...
        · intro f g disj intf intg
          ...
          conv_lhs =>
           ...
           intro x
           rw [mul_add]
          have disj':  Disjoint (Function.support ((B.indicator 1)*f)) (Function.support ((B.indicator 1)*g)) := by
            simp
            refine (Disjoint.inter_left' B ?_)
            refine (Disjoint.inter_right' B ?_)
            exact disj
          have intBf: Integrable (μ := (π x₀))  (((B.indicator 1)*f) : X → ℝ ) := by sorry
          have intBg: Integrable (μ := (π x₀))  (((B.indicator 1)*g) : X → ℝ ) := by sorry
          ...
          have fun_add: ∀x, (B.indicator 1 x * f x + B.indicator 1 x * g x
           = (((B.indicator 1)*f):X → ℝ) x + (((B.indicator 1)*g):X → ℝ) x ) := by
            ...
            simp
          simp [fun_add]
          rw [integral_add intBf intBg]
          simp
          rw [hp1,hp2]
          ring
        · refine IsComplete.isClosed ?_
          intro f1 cf1 fleF
          have := memLp_one_iff_integrable.mpr hf
          have := this.toLp
          use this
          simp
          sorry
